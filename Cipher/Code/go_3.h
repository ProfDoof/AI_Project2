...
no pruned
uniFit considered
double mutaion (.05) every 200 add best
elitism
unierr * 1
dierr * 1.5
trifit * 3
...
{   
    ...
    for (int i = 0; i < key.size(); i++) {
        ...
        uniFit += ((1.0 * count(translated.begin(), translated.end(), key[i]) / translated.size()) / index);
    }
    ...
    return 5 - (factor * ((uniError * uniFit )+ (1.5 * diError))) + (3 * triFit) + uniFit;
}
...
void GACipher::run(std::string filename)
{
    ...
    //elem.first = "NRJTAPYXLHGCWUODIKBVEQMFSZ";
